class BwLogFormatter
  def self.log_hash(data, datetime=nil)
    ...
    ActiveSupport::OrderedHash.new.tap do |h|
      ...
      h[:duration] = timings[:total].to_f if timings && timings.has_key?(:total)
      h[:db] = timings[:db].to_f if timings && timings.has_key?(:db)
      h[:view] = timings[:view].to_f if timings && timings.has_key?(:view)
